	...
	<view>
		...
		<view v-if="navs && navs.length > 0" class="nav-list">
			<block v-for="(item,index) in navs" :key="index">
				<view class="nav-item">
					<view class="nav-pic" @click="onNavTap" :data-index="index">
			...
			<view class="weui-cell hosp-item weui-cell_access" v-for="(item,index) in hospitals" :key="item.id"
				...
				<view class="weui-cell__hd">
					<image class="hosp-avatar"
						:src="item.avatar ? item.avatar_url : '../../static/resouce/images/avatar.ipg'"
...
<script setup>
	...
	const toHospital = (e) => {
		uni.navigateTo({
			url: '/pages/hospital/index?hid=' + e.currentTarget.dataset
		})
	}
</script>
